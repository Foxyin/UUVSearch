...
class SACTrainer:
    def __init__(self, env, config, exp_name="sac"):
        obs_dim = env.observation_space.shape[0]
        action_dim = env.action_space.n
        self.env = env
        self.agent = SACAgent(obs_dim, action_dim, config["algo"])
    ...
    def train(self):
        ...
        for step in range(self.total_steps):
            ...
            if done:
                ...
                if loss_info:
                    self.logger.log_scalar("train/loss_critic", loss_info["loss_critic"], step)
                    self.logger.log_scalar("train/loss_actor", loss_info["loss_actor"], step)
                    self.logger.log_scalar("train/alpha", loss_info["alpha"], step)
                obs, info = self.env.reset()
                episode_reward = 0
                episode_len = 0
