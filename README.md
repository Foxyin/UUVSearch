...
| `target_reward_only`    | 0 | 0 | 100 | 0 | 0 |
...
拼接为扁平向量，输入维度 = 3×(2R+1)² + 3（默认 R=5，共 366 维）。

### 环境对比

| | 网格环境 (GridEnv) | 连续环境 (ContinuousSearchEnv) |
|---|---|---|
| **运动** | 逐格移动 | 一阶运动学（v·dt = 60m/步） |
| **动作** | 8 方向（罗盘） | 5 航向变化（-90°~+90°） |
| **观测** | dict（含 auv_pos, coverage 等） | 1D array（366 维 patch） |
...
| DQN | Value RL | — | ✅ 5 动作 | target network + ε-greedy |
| SAC | Actor-Critic RL | — | ✅ 5 动作 | 双 Critic + 自动熵调节 |

> 传统算法与 RL 算法在**同一连续环境**下对比，保证公平性。
...
| 问题 | 说明 | 改进方向 |
|------|------|----------|
| **概率更新简化** | 当前使用 ×0.5/×2.0 启发式，非严格贝叶斯推断 | 引入传感器模型（检测概率 P(D\|T)、虚警率），实现真贝叶斯更新 |
| **不确定度不增长** | 探测后归零，永不恢复；长时间任务中 uncertain map 失效 | 添加遗忘因子或时间衰减，恢复未重访区域的不确定度 |
...
| **真实声呐模型** | 替换当前简化 FOV 为频率相关声传播模型（射线追踪/抛物方程） |
